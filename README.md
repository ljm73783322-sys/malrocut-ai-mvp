...
- 시스템에 `FFmpeg`가 설치되어 있으면 화면 보정(밝기 증대, 약간 확대, 1080x1920 리사이징) 및 실제 썸네일 추출 기본 변환을 수행합니다.
- `FFmpeg`가 없을 경우 시스템 장애를 방지하기 위해 **원본 MP4 파일을 그대로 복사**하여 다운로드 시 정상 재생이 가능한 결과 파일로 반환하는 Fallback 로직을 사용합니다.
