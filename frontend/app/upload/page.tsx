...
export default function UploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await analyzeVideo(job_id);
      router.push(`/analyze/${job_id}`);
    } catch (e) {
      alert("업로드에 실패했습니다. 백엔드 서버가 실행 중인지 확인하세요.");
      setLoading(false);
    }
  };
    ...
    <div className="flex flex-col items-center space-y-10 py-10">
      ...
      <button 
        ...
        className={`text-3xl font-bold py-6 px-16 rounded-2xl shadow-lg transition ${
          file && !loading ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-300 text-gray-500 cursor-not-allowed'
        }`}
      >
